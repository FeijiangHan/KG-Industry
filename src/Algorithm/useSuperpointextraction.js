...
*/
function pre_handle_process (nodes, links) {
  ...
  global.nodes = nodes
  global.links = links
  const { nodeDegreeMap, nodeNeighbor, nodeSet, linkMap
    , nodeLinkMap } = global
  nodes.forEach(node => {
...
/*
  生成超点
*/
function generateAbstractNode (nodeArray, type = 'super') {
  const { linkMap, nodeLinkMap } = global
  const childrenLinks = []

  nodeArray.forEach(node => {

    nodeLinkMap.get(node.id).forEach(linkId => {
...
export default function superpointextraction (nodes, links) {
  pre_handle_process(nodes, links)
  get_cluster()
  get_bridge()
  //get_doubleBridge()  本项目不需要该结构 
  const { linkMap, nodeSet } = global
  return {
